package com.kautec.applicationcontentprovider.database
...
import com.kautec.applicationcontentprovider.database.NotesDatabaseHelper.Companion.TABLE_NOTES
...
class NotesProvider : ContentProvider() {
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        if (mUriMatcher.match(uri) == NOTES_BY_ID) {
            ...
            val lineAffect = db.delete(TABLE_NOTES, "$_ID =?", arrayOf(uri.lastPathSegment))
            db.close()
    ...
    //Usado para requisição de arquivos
    ...
    override fun update(
        ...
        if (mUriMatcher.match(uri) == NOTES_BY_ID){
            ...
            throw UnsupportedSchemeException("Uri não implementada no update")
        }
    }

    companion object   {
        const val AUTHORITY = "com.kautec.applicationcontentprovider.provider"
